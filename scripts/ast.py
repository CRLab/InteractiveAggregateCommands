...
class GoTo(Task):
	...
	def __str__(self):
		return "Goto { location : \"" + self.location_name + "\"}"
...
class UnprocessedExecute(Command):
	def __init__(self, task_list):
		self.task_list = task_list

	def __str__(self):
		return "UnprocessedExecute : { name : \"" +  + "\" }"

	def pretty_print(self):
		return "execute " + self.task_list

class UnprocessedRecord(Command):
	...
	def __str__(self):
		return "UnprocessedRecord : { name : \"" +  + "\" }"

	def pretty_print(self):
		return "record " + self.task_list + " as " + self.task_name